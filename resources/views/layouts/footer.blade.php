<footer class="bg-green-700 text-white text-sm px-6 py-6">
    <div class="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">

        <!-- Address -->
        <div>
            <h4 class="font-semibold mb-1 flex items-center gap-2">
                <i class="fas fa-map-marker-alt"></i> Address
            </h4>
            <p>Room No.(7–A), (7) floor<br>
                Mawyawadi Condo<br>
                8 Mile, Mayangone, Yangon
            </p>
        </div>

        <!-- Contact -->
        <div>
            <h4 class="font-semibold mb-1 flex items-center gap-2">
                <i class="fa-solid fa-tty"></i> Contact
            </h4>
            <p><i class="fas fa-phone mr-1"></i> +959 765070160<br>
                <i class="fas fa-phone mr-1"></i> +959 965070160
            </p>
            <p class="mt-2"><i class="fas fa-clock mr-1"></i> Monday to Saturday (9:00 AM – 5:00 PM)</p>
            <p class="mt-1">
                <i class="fas fa-envelope mr-1"></i>
                <a href="mailto:unitysourceteam@gmail.com" class="underline">unitysourceteam@gmail.com</a>
            </p>
        </div>

    </div>
</footer>
